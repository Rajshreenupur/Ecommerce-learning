...
export const createProduct = async (
  ...
  if (
    ...
    res.status(400).json({
      message:
        "Category,Product-Name,description,sizesQuantities and Product-Price  are required",
    });
    return;
  }

  try {
    if (!sizesQuantities || sizesQuantities.length === 0) {
      res.status(400).json({ message: "Sizes and quantities are required." });
    }
...
export const getAllProduct = async (
  ...
  try {
    ...
    if (!product) {
      res.status(404).json({ message: "No product found for this user" });
    }
...
export const updateProduct = async (
  ...
  try {
    ...
    if (!productExists) {
      ...
      const updateProductResult = await ProductCategory.findByIdAndUpdate(
        productId,
        payload,
        { new: true }
      );
...
export const deleteProduct = async (
  ...
  try {
    ...
    if (!productExists) {
      ...
      const deleteProductResult = await ProductCategory.findByIdAndDelete(
        productId
      );
      res
        .status(200)
        .json({ message: "Product deleted successful", deleteProductResult });
    }
  } catch (error) {
    res.status(500).json({
